<?php
  ...
  function monta_select_curso()
  {
      ...
      if ($result = mysqli_query($link, $query)) {
          ...
          while ($row = mysqli_fetch_assoc($result)) {
              ...
              $data_inicio = $row["dt_inicio"];
  
              echo "<option value=\"$id\">";
              echo $curso . ' - Carga Horária: ' . $carga_horaria . ' - Data de Início: ' . $data_inicio . '</option>';
          }
          echo "</select>";
  ...
  function recupera_curso($id){
    global $link;
    // lista cursos já cadastrados
    $query = "SELECT id_curso, ds_curso, dt_inicio, nr_carga_horaria FROM tb_curso WHERE id_curso = $id;";
    if ($result = mysqli_query($link, $query)) {
      // busca os dados lidos do banco de dados
      while ($row = mysqli_fetch_assoc($result)) {
        return $row;
